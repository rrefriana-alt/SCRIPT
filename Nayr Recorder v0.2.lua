...
local ShiftLockBtnControl, ResetBtnControl, ShowRuteBtnControl, SmartAutoWalkBtnControl
...
local ProgressFill, PlaybackProgress
local MainContentScrollingFrame -- ⭐ NEW: Scrolling frame untuk main content
...
local SmartAutoWalkEnabled = false
...
-- ========= FIXED SMART NEAREST FRAME =========
local function FindNearestFrameToStart(recording, currentPos)
    if not recording or #recording == 0 then return 1 end
    
    local nearestIndex = 1
    local minDistance = math.huge
    
    -- Cari frame yang posisinya paling dekat dengan karakter
    ...
    print(string.format("🎯 Smart Start: Frame %d/%d (Distance: %.2f studs)", nearestIndex, #recording, minDistance))
    
    return nearestIndex  -- ⭐ Return frame yang benar-benar terdekat, TANPA dikurangi buffer
end
    ...
    SafeCall(function()
        if nearestRecording and distance <= 50 then
            ...
            if distanceInt <= 10 then
                buttonColor = Color3.fromRGB(40, 180, 80)
            elseif distanceInt <= 30 then
                buttonColor = Color3.fromRGB(200, 180, 50)
            else
                buttonColor = Color3.fromRGB(255, 140, 50)
            end
...
local function ToggleSmartAutoWalk()
    SmartAutoWalkEnabled = not SmartAutoWalkEnabled
    
    if SmartAutoWalkBtnControl then
        if SmartAutoWalkEnabled then
            SmartAutoWalkBtnControl.Text = "Smart ON"
            SmartAutoWalkBtnControl.BackgroundColor3 = Color3.fromRGB(40, 180, 80)
            PlaySound("Success")
        else
            SmartAutoWalkBtnControl.Text = "Smart OFF"
            SmartAutoWalkBtnControl.BackgroundColor3 = Color3.fromRGB(80, 80, 80)
            PlaySound("Toggle")
        end
    end
end
...
local function PlayFromSpecificFrame(recording, startFrame, recordingName)
    ...
    -- ⭐ FIXED: Dapatkan target frame yang benar
    ...
    -- ⭐ PERBAIKAN: JANGAN teleport kalau distance kecil (biarkan natural)
    if distance > 10 then  -- Hanya teleport jika jarak > 10 studs
        hrp.CFrame = GetFrameCFrame(targetFrame)
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        print(string.format("📍 Teleport to frame %d (Distance: %.2f studs)", startFrame, distance))
        task.wait(0.03)
    else
        print(string.format("🚶 Starting from frame %d (Already near: %.2f studs)", startFrame, distance))
    end
...
local function SmartPlayRecording(maxDistance)
    ...
    for _, recordingName in ipairs(RecordingOrder) do
        ...
        if recording and #recording > 0 then
            ...
            if frameDistance < bestDistance and frameDistance <= (maxDistance or 50) then
                ...
                -- ⭐ PERBAIKAN BESAR: Gunakan fungsi FindNearestFrameToStart yang benar
                if SmartAutoWalkEnabled then
                    bestFrame = FindNearestFrameToStart(recording, currentPos)
                    print(string.format("🎯 SMART MODE: Starting from frame %d (Distance: %.2f studs)", bestFrame, bestDistance))
                else
                    bestFrame = 1  -- Normal mode: mulai dari awal
                    print(string.format("🎮 NORMAL MODE: Starting from frame 1"))
                end
    ...
    if bestRecording then
        ...
        PlaySound("Error")
        print("❌ No recording found within range")
    end
end
...
local function StartTitlePulse(titleLabel)
    ...
    local fullText = "Ihaans Recorder v.01"
    local letters = {}
    ...
    titlePulseConnection = RunService.RenderStepped:Connect(function()
        pcall(function()
            ...
            for i, letter in ipairs(letters) do
                local offset = math.sin(now * 3 + i * 0.5) * 8
...
local function SaveToObfuscatedJSON()
    ...
    if filename == "" then filename = "Ihaans" end
...
function UpdateRecordList()
    SafeCall(function()
        ...
        for index, name in ipairs(RecordingOrder) do
            ...
            card.Size = UDim2.new(1, -6, 0, 95)  -- Slightly taller for better layout
            card.Position = UDim2.new(0, 3, 0, yPos)
            card.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
            card.BorderSizePixel = 0
            card.Parent = RecordingsList
            ...
            cardShadow.ImageColor3 = Color3.fromRGB(59, 15, 116)
            ...
            borderStroke.Color = Color3.fromRGB(59, 15, 116)
            borderStroke.Thickness = 1.5
            borderStroke.Transparency = 0.3
            borderStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            borderStroke.Parent = cardBorder
            
            -- Animate border color
            task.spawn(function()
                while cardBorder and cardBorder.Parent do
                    for i = 0, 1, 0.05 do
                        if not cardBorder or not cardBorder.Parent then break end
            ...
            cardHeader.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
            ...
            headerGradient.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Color3.fromRGB(59, 15, 116)),
                ColorSequenceKeypoint.new(0.5, Color3.fromRGB(100, 50, 200)),
                ColorSequenceKeypoint.new(1, Color3.fromRGB(59, 15, 116))
            })
            headerGradient.Rotation = 90
            headerGradient.Parent = cardHeader
            
            -- Checkbox with animation
            local miniCheck = Instance.new("TextButton")
            miniCheck.Size = UDim2.fromOffset(20, 20)
            miniCheck.Position = UDim2.fromOffset(5, 5)
            miniCheck.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
            miniCheck.Text = CheckedRecordings[name] and "✓" or ""
            miniCheck.TextColor3 = CheckedRecordings[name] and Color3.fromRGB(100, 255, 150) or Color3.fromRGB(150, 150, 150)
            miniCheck.Font = Enum.Font.GothamBold
            miniCheck.TextSize = 14
            miniCheck.BorderSizePixel = 0
            miniCheck.Parent = cardHeader
            
            local checkCorner = Instance.new("UICorner")
            checkCorner.CornerRadius = UDim.new(0, 4)
            checkCorner.Parent = miniCheck
            
            local checkStroke = Instance.new("UIStroke")
            checkStroke.Color = CheckedRecordings[name] and Color3.fromRGB(100, 255, 150) or Color3.fromRGB(80, 80, 90)
            checkStroke.Thickness = 1.5
            checkStroke.Parent = miniCheck
            
            -- Name input with placeholder styling
            local nameLabel = Instance.new("TextBox")
            nameLabel.Size = UDim2.new(1, -30, 1, 0)
            nameLabel.Position = UDim2.fromOffset(30, 0)
            nameLabel.BackgroundTransparency = 1
            nameLabel.Text = checkpointNames[name] or "Checkpoint " .. index
            nameLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            nameLabel.Font = Enum.Font.GothamBold
            nameLabel.TextSize = 11
            nameLabel.TextXAlignment = Enum.TextXAlignment.Left
            nameLabel.PlaceholderText = "Rename checkpoint..."
            nameLabel.PlaceholderColor3 = Color3.fromRGB(150, 150, 150)
            nameLabel.ClearTextOnFocus = false
            nameLabel.Parent = cardHeader
            
            -- Stats section
            local statsFrame = Instance.new("Frame")
            statsFrame.Size = UDim2.new(1, -10, 0, 30)
            statsFrame.Position = UDim2.fromOffset(5, 35)
            statsFrame.BackgroundTransparency = 1
            statsFrame.Parent = card
            
            local duration = #rec > 0 and FormatDuration(rec[#rec].Timestamp) or "0:00"
            local frames = #rec
            
            local statsText = Instance.new("TextLabel")
            statsText.Size = UDim2.new(1, 0, 1, 0)
            statsText.BackgroundTransparency = 1
            statsText.Text = string.format("⏱ %s  |  📊 %d frames", duration, frames)
            ...
            actionsFrame.Size = UDim2.new(1, -10, 0, 25)
            actionsFrame.Position = UDim2.fromOffset(5, 65)
            actionsFrame.BackgroundTransparency = 1
            actionsFrame.Parent = card
            
            local playBtnSmall = Instance.new("TextButton")
            playBtnSmall.Size = UDim2.new(0.48, 0, 1, 0)
            playBtnSmall.Position = UDim2.fromOffset(0, 0)
            playBtnSmall.BackgroundColor3 = Color3.fromRGB(59, 15, 116)
            playBtnSmall.Text = "▶ PLAY"
            playBtnSmall.TextColor3 = Color3.new(1, 1, 1)
            playBtnSmall.Font = Enum.Font.GothamBold
            playBtnSmall.TextSize = 10
            playBtnSmall.BorderSizePixel = 0
            playBtnSmall.Parent = actionsFrame
            
            local delBtnSmall = Instance.new("TextButton")
            delBtnSmall.Size = UDim2.new(0.48, 0, 1, 0)
            delBtnSmall.Position = UDim2.new(0.52, 0, 0, 0)
            delBtnSmall.BackgroundColor3 = Color3.fromRGB(200, 50, 60)
            delBtnSmall.Text = "🗑 DELETE"
            delBtnSmall.TextColor3 = Color3.new(1, 1, 1)
            delBtnSmall.Font = Enum.Font.GothamBold
            delBtnSmall.TextSize = 10
            delBtnSmall.BorderSizePixel = 0
            delBtnSmall.Parent = actionsFrame
            
            -- Button corners and effects
            for _, btn in pairs({playBtnSmall, delBtnSmall}) do
            ...
            -- Event handlers
            miniCheck.MouseButton1Click:Connect(function()
                CheckedRecordings[name] = not CheckedRecordings[name]
                miniCheck.Text = CheckedRecordings[name] and "✓" or ""
                miniCheck.TextColor3 = CheckedRecordings[name] and Color3.fromRGB(100, 255, 150) or Color3.fromRGB(150, 150, 150)
                checkStroke.Color = CheckedRecordings[name] and Color3.fromRGB(100, 255, 150) or Color3.fromRGB(80, 80, 90)
                AnimateButtonClick(miniCheck)
            end)
            
            nameLabel.FocusLost:Connect(function()
                if nameLabel.Text ~= "" then
                    checkpointNames[name] = nameLabel.Text
                    PlaySound("Success")
                end
            end)
            
            playBtnSmall.MouseButton1Click:Connect(function()
                if not IsPlaying and not IsAutoLoopPlaying then
                    AnimateButtonClick(playBtnSmall)
                    PlayRecording(name)
                end
            end)
            
            delBtnSmall.MouseButton1Click:Connect(function()
                AnimateButtonClick(delBtnSmall)
            ...
            playBtnSmall.MouseEnter:Connect(function()
                TweenService:Create(playBtnSmall, TweenInfo.new(0.2), {
                    BackgroundColor3 = Color3.fromRGB(80, 25, 150),
                    Size = UDim2.new(0.49, 0, 1.05, 0)
                }):Play()
            end)
            
            playBtnSmall.MouseLeave:Connect(function()
                TweenService:Create(playBtnSmall, TweenInfo.new(0.2), {
            ...
            delBtnSmall.MouseEnter:Connect(function()
                TweenService:Create(delBtnSmall, TweenInfo.new(0.2), {
                    BackgroundColor3 = Color3.fromRGB(230, 70, 80),
                    Size = UDim2.new(0.49, 0, 1.05, 0)
                }):Play()
            end)
            
            delBtnSmall.MouseLeave:Connect(function()
                TweenService:Create(delBtnSmall, TweenInfo.new(0.2), {
            ...
            yPos = yPos + 100  -- Increased spacing between cards
        end
...
-- ========= UI CREATION (WITH SCROLLABLE CONTENT) =========

local uiSuccess, uiError = pcall(function()
    ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "Ihaans Recorder v.01"
    ScreenGui.ResetOnSpawn = false
    ...
    MainFrame.Size = UDim2.fromOffset(320, 450)  -- Increased height for scroll
    ...
    -- Main shadow with glow
    local MainShadow = Instance.new("ImageLabel")
    MainShadow.Name = "Shadow"
    MainShadow.Size = UDim2.new(1, 15, 1, 15)
    MainShadow.Position = UDim2.new(0, -7.5, 0, -7.5)
    MainShadow.BackgroundTransparency = 1
    MainShadow.Image = "rbxassetid://1316045217"
    MainShadow.ImageColor3 = Color3.fromRGB(59, 15, 116)
    ...
    HeaderGradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(59, 15, 116)),
        ColorSequenceKeypoint.new(0.3, Color3.fromRGB(100, 50, 200)),
        ColorSequenceKeypoint.new(0.7, Color3.fromRGB(150, 75, 255)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(59, 15, 116))
    })
    HeaderGradient.Rotation = 90
    ...
    -- ========= SCROLLABLE CONTENT =========
    MainContentScrollingFrame = Instance.new("ScrollingFrame")
    MainContentScrollingFrame.Size = UDim2.new(1, 0, 1, -45)
    MainContentScrollingFrame.Position = UDim2.new(0, 0, 0, 45)
    MainContentScrollingFrame.BackgroundTransparency = 1
    MainContentScrollingFrame.BorderSizePixel = 0
    MainContentScrollingFrame.ScrollBarThickness = 6
    MainContentScrollingFrame.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 255)
    MainContentScrollingFrame.ScrollingDirection = Enum.ScrollingDirection.Y
    MainContentScrollingFrame.VerticalScrollBarInset = Enum.ScrollBarInset.Always
    MainContentScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, 650)  -- Height akan diupdate
    MainContentScrollingFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
    MainContentScrollingFrame.Parent = MainFrame

    local Content = Instance.new("Frame")
    Content.Size = UDim2.new(1, -12, 0, 650)  -- Temporary height
    Content.Position = UDim2.new(0, 6, 0, 6)
    Content.BackgroundTransparency = 1
    Content.Parent = MainContentScrollingFrame

    -- ========= CONTROL SECTION =========
    local ControlSection = Instance.new("Frame")
    ControlSection.Size = UDim2.new(1, 0, 0, 35)
    ControlSection.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    ...
    ControlStroke.Color = Color3.fromRGB(59, 15, 116)
    ...
    local function CreateControlBtn(text, x, size, color)
        ...
        btn.Size = UDim2.fromOffset(size, 27)
        ...
        btn.MouseEnter:Connect(function()
            TweenService:Create(btn, TweenInfo.new(0.2), {
                ...
                Size = UDim2.fromOffset(size + 2, 29)
            }):Play()
        end)
    ...
    local PlayBtn = CreateControlBtn("🎮 PLAY", 0, 100, Color3.fromRGB(59, 15, 116))
    local RecordBtn = CreateControlBtn("⏺ RECORD", 104, 100, Color3.fromRGB(59, 15, 116))
    local MenuBtn = CreateControlBtn("⚙️ MENU", 208, 100, Color3.fromRGB(59, 15, 116))

    -- ========= SPEED & WALKSPEED SECTION =========
    local SpeedWalkSection = Instance.new("Frame")
    SpeedWalkSection.Size = UDim2.new(1, 0, 0, 70)
    SpeedWalkSection.Position = UDim2.new(0, 0, 0, 45)
    SpeedWalkSection.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    SpeedWalkSection.BorderSizePixel = 0
    SpeedWalkSection.Parent = Content

    local SpeedWalkCorner = Instance.new("UICorner")
    SpeedWalkCorner.CornerRadius = UDim.new(0, 8)
    SpeedWalkCorner.Parent = SpeedWalkSection

    local SpeedWalkStroke = Instance.new("UIStroke")
    SpeedWalkStroke.Color = Color3.fromRGB(59, 15, 116)
    SpeedWalkStroke.Thickness = 1.5
    SpeedWalkStroke.Parent = SpeedWalkSection

    -- Speed Input
    local speedLabel = Instance.new("TextLabel")
    speedLabel.Size = UDim2.new(0.3, -5, 0, 20)
    speedLabel.Position = UDim2.fromOffset(5, 10)
    ...
    speedLabel.Parent = SpeedWalkSection

    SpeedBox = Instance.new("TextBox")
    SpeedBox.Size = UDim2.new(0.3, -5, 0, 25)
    SpeedBox.Position = UDim2.fromOffset(5, 30)
    ...
    local filenameStroke = Instance.new("UIStroke")
    filenameStroke.Color = Color3.fromRGB(100, 100, 255)
    filenameStroke.Thickness = 1
    filenameStroke.Parent = FilenameBox

    -- WalkSpeed Input
    local walkSpeedLabel = Instance.new("TextLabel")
    walkSpeedLabel.Size = UDim2.new(0.3, -5, 0, 20)
    walkSpeedLabel.Position = UDim2.new(0.8, 5, 0, 10)
    ...
    walkSpeedLabel.Parent = SpeedWalkSection

    WalkSpeedBox = Instance.new("TextBox")
    WalkSpeedBox.Size = UDim2.new(0.3, -5, 0, 25)
    WalkSpeedBox.Position = UDim2.new(0.8, 5, 0, 30)
    ...
    walkSpeedStroke.Color = Color3.fromRGB(100, 100, 255)
    walkSpeedStroke.Thickness = 1
    walkSpeedStroke.Parent = WalkSpeedBox

    -- ========= SAVE SECTION =========
    local SaveSection = Instance.new("Frame")
    SaveSection.Size = UDim2.new(1, 0, 0, 80)
    SaveSection.Position = UDim2.new(0, 0, 0, 125)
    SaveSection.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    ...
    SaveStroke.Color = Color3.fromRGB(59, 15, 116)
    SaveStroke.Thickness = 1.5
    SaveStroke.Parent = SaveSection

    local SaveButtons = Instance.new("Frame")
    SaveButtons.Size = UDim2.new(1, -8, 1, -8)
    SaveButtons.Position = UDim2.new(0, 4, 0, 4)
    SaveButtons.BackgroundTransparency = 1
    SaveButtons.Parent = SaveSection

    local SaveFileBtn = CreateControlBtn("💾 SAVE", 0, 100, Color3.fromRGB(59, 15, 116))
    SaveFileBtn.Parent = SaveButtons
    SaveFileBtn.Position = UDim2.fromOffset(0, 5)

    local MergeBtn = CreateControlBtn("🔀 MERGE", 104, 100, Color3.fromRGB(59, 15, 116))
    MergeBtn.Parent = SaveButtons
    MergeBtn.Position = UDim2.fromOffset(104, 5)

    local LoadFileBtn = CreateControlBtn("📂 LOAD", 208, 100, Color3.fromRGB(59, 15, 116))
    LoadFileBtn.Parent = SaveButtons
    LoadFileBtn.Position = UDim2.fromOffset(208, 5)

    CheckAllBtn = Instance.new("TextButton")
    CheckAllBtn.Size = UDim2.new(1, 0, 0, 30)
    CheckAllBtn.Position = UDim2.fromOffset(0, 45)
    ...
    CheckAllBtn.TextSize = 12
    CheckAllBtn.AutoButtonColor = false
    CheckAllBtn.Parent = SaveButtons

    local CheckAllCorner = Instance.new("UICorner")
    CheckAllCorner.CornerRadius = UDim.new(0, 6)
    CheckAllCorner.Parent = CheckAllBtn

    local checkAllStroke = Instance.new("UIStroke")
    checkAllStroke.Color = Color3.fromRGB(255, 255, 255)
    checkAllStroke.Thickness = 0.8
    checkAllStroke.Transparency = 0.8
    checkAllStroke.Parent = CheckAllBtn
    ...
    RecordingsSection.Size = UDim2.new(1, 0, 0, 250)
    RecordingsSection.Position = UDim2.new(0, 0, 0, 215)
    RecordingsSection.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    RecordingsSection.BorderSizePixel = 0
    RecordingsSection.Parent = Content

    local RecordingsCorner = Instance.new("UICorner")
    RecordingsCorner.CornerRadius = UDim.new(0, 8)
    RecordingsCorner.Parent = RecordingsSection

    local RecordingsStroke = Instance.new("UIStroke")
    RecordingsStroke.Color = Color3.fromRGB(59, 15, 116)
    RecordingsStroke.Thickness = 1.5
    RecordingsStroke.Parent = RecordingsSection

    local recordingsTitle = Instance.new("TextLabel")
    recordingsTitle.Size = UDim2.new(1, 0, 0, 25)
    recordingsTitle.Position = UDim2.fromOffset(5, 5)
    recordingsTitle.BackgroundTransparency = 1
    recordingsTitle.Text = "📁 RECORDINGS"
    recordingsTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
    recordingsTitle.Font = Enum.Font.GothamBold
    recordingsTitle.TextSize = 14
    recordingsTitle.TextXAlignment = Enum.TextXAlignment.Left
    recordingsTitle.Parent = RecordingsSection

    RecordingsList = Instance.new("ScrollingFrame")
    RecordingsList.Size = UDim2.new(1, -10, 1, -35)
    RecordingsList.Position = UDim2.new(0, 5, 0, 30)
    RecordingsList.BackgroundColor3 = Color3.fromRGB(20, 20, 30)
    RecordingsList.BorderSizePixel = 0
    RecordingsList.ScrollBarThickness = 6
    RecordingsList.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 255)
...
local miniButtonShadow = Instance.new("ImageLabel")
miniButtonShadow.Size = UDim2.new(1, 10, 1, 10)
miniButtonShadow.Position = UDim2.new(0, -5, 0, -5)
miniButtonShadow.BackgroundTransparency = 1
miniButtonShadow.Image = "rbxassetid://1316045217"
miniButtonShadow.ImageColor3 = Color3.fromRGB(59, 15, 116)
miniButtonShadow.ImageTransparency = 0.6
miniButtonShadow.ScaleType = Enum.ScaleType.Slice
miniButtonShadow.SliceCenter = Rect.new(10, 10, 118, 118)
miniButtonShadow.Parent = MiniButton

-- Mini Button Ultimate Animation
do
    local letters = {"B", "Y", "N", "A", "Y", "R"}
    ...
    local ultimateAnimConn = RunService.RenderStepped:Connect(function()
        if MiniButton and MiniButton.Parent then
            ...
            -- Pulsing scale
            local scale = 30 + math.sin(now * 3) * 4
            MiniButton.TextSize = scale
            
            -- Subtle rotation
            local angle = math.sin(now * 1.5) * 10
...
PlaybackControl.Size = UDim2.fromOffset(180, 140)
PlaybackControl.Position = UDim2.new(0.5, -90, 0.5, -70)
...
PlaybackCorner.CornerRadius = UDim.new(0, 12)
PlaybackCorner.Parent = PlaybackControl

local PlaybackShadow = Instance.new("ImageLabel")
PlaybackShadow.Size = UDim2.new(1, 10, 1, 10)
PlaybackShadow.Position = UDim2.new(0, -5, 0, -5)
PlaybackShadow.BackgroundTransparency = 1
PlaybackShadow.Image = "rbxassetid://1316045217"
PlaybackShadow.ImageColor3 = Color3.fromRGB(59, 15, 116)
PlaybackShadow.ImageTransparency = 0.7
PlaybackShadow.ScaleType = Enum.ScaleType.Slice
PlaybackShadow.SliceCenter = Rect.new(10, 10, 118, 118)
PlaybackShadow.Parent = PlaybackControl

    local PlaybackContent = Instance.new("Frame")
    PlaybackContent.Size = UDim2.new(1, -8, 1, -8)
    PlaybackContent.Position = UDim2.new(0, 4, 0, 4)
    ...
    PlayBtnControl = CreatePlaybackBtn("▶ PLAY", 3, 3, 170, 25, Color3.fromRGB(59, 15, 116))
    LoopBtnControl = CreatePlaybackBtn("🔄 Loop OFF", 3, 31, 83, 20, Color3.fromRGB(80, 80, 80))
    JumpBtnControl = CreatePlaybackBtn("🦘 Jump OFF", 89, 31, 84, 20, Color3.fromRGB(80, 80, 80))
    RespawnBtnControl = CreatePlaybackBtn("💀 Respawn OFF", 3, 54, 83, 20, Color3.fromRGB(80, 80, 80))
    ShiftLockBtnControl = CreatePlaybackBtn("🔒 Shift OFF", 89, 54, 84, 20, Color3.fromRGB(80, 80, 80))
    ResetBtnControl = CreatePlaybackBtn("🔄 Reset OFF", 3, 77, 83, 20, Color3.fromRGB(80, 80, 80))
    ShowRuteBtnControl = CreatePlaybackBtn("🗺️ Rute OFF", 89, 77, 84, 20, Color3.fromRGB(80, 80, 80))
    SmartAutoWalkBtnControl = CreatePlaybackBtn("🎯 Smart OFF", 3, 100, 170, 20, Color3.fromRGB(80, 80, 80))

    -- Progress Bar
PlaybackProgress = Instance.new("Frame")
PlaybackProgress.Size = UDim2.new(1, -10, 0, 6)
PlaybackProgress.Position = UDim2.fromOffset(5, 123)
PlaybackProgress.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
PlaybackProgress.BorderSizePixel = 0
PlaybackProgress.Parent = PlaybackContent

local ProgressCorner = Instance.new("UICorner")
ProgressCorner.CornerRadius = UDim.new(1, 0)
ProgressCorner.Parent = PlaybackProgress

ProgressFill = Instance.new("Frame")
ProgressFill.Size = UDim2.new(0, 0, 1, 0)
ProgressFill.BackgroundColor3 = Color3.fromRGB(59, 15, 116)
ProgressFill.BorderSizePixel = 0
ProgressFill.Parent = PlaybackProgress
...
FillCorner.Parent = ProgressFill
    ...
    RecordingStudio.Size = UDim2.fromOffset(180, 140)
    RecordingStudio.Position = UDim2.new(0.5, -90, 0.5, -70)
    ...
    StudioCorner.CornerRadius = UDim.new(0, 12)
    StudioCorner.Parent = RecordingStudio

    local StudioShadow = Instance.new("ImageLabel")
    StudioShadow.Size = UDim2.new(1, 10, 1, 10)
    StudioShadow.Position = UDim2.new(0, -5, 0, -5)
    StudioShadow.BackgroundTransparency = 1
    StudioShadow.Image = "rbxassetid://1316045217"
    StudioShadow.ImageColor3 = Color3.fromRGB(59, 15, 116)
    StudioShadow.ImageTransparency = 0.7
    StudioShadow.ScaleType = Enum.ScaleType.Slice
    StudioShadow.SliceCenter = Rect.new(10, 10, 118, 118)
    StudioShadow.Parent = RecordingStudio

    local StudioContent = Instance.new("Frame")
    StudioContent.Size = UDim2.new(1, -8, 1, -8)
    StudioContent.Position = UDim2.new(0, 4, 0, 4)
    ...
    SaveBtn = CreateStudioBtn("💾 SAVE", 3, 3, 85, 22, Color3.fromRGB(59, 15, 116))
    StartBtn = CreateStudioBtn("⏺ START", 92, 3, 85, 22, Color3.fromRGB(59, 15, 116))
    ResumeBtn = CreateStudioBtn("▶️ LANJUTKAN", 3, 28, 174, 22, Color3.fromRGB(59, 15, 116))
    PrevBtn = CreateStudioBtn("◀ MUNDUR", 3, 58, 85, 30, Color3.fromRGB(59, 15, 116))
...
NextBtn = CreateStudioBtn("MAJU ▶", 92, 58, 85, 30, Color3.fromRGB(59, 15, 116))
...
NextBtn.InputEnded:Connect(function(input)
    ...
    if IsPlaying or IsAutoLoopPlaying then
        StopPlayback()
        PlayBtnControl.Text = "RESUME"
        PlayBtnControl.BackgroundColor3 = Color3.fromRGB(40, 180, 80)
    else
        if AutoLoop then
            StartAutoLoopAll()
        else
    ...
    ShowRuteBtnControl.MouseButton1Click:Connect(function()
        ...
        if ShowPaths then
            ShowRuteBtnControl.Text = "🗺️ Path ON"
            ShowRuteBtnControl.BackgroundColor3 = Color3.fromRGB(40, 180, 80)
            PathsHiddenOnce = false
            PathHasBeenUsed = {}
            VisualizeAllPaths()
        else
            ShowRuteBtnControl.Text = "🗺️ Path OFF"
            ShowRuteBtnControl.BackgroundColor3 = Color3.fromRGB(80, 80, 80)
            ClearPathVisualization()
        end
    end)

    SmartAutoWalkBtnControl.MouseButton1Click:Connect(function()
        AnimateButtonClick(SmartAutoWalkBtnControl)
        ToggleSmartAutoWalk()
    end)
...
-- ========= MINI BUTTON: MOBILE-SAFE DRAG + FIVE TAP =========
local tapCount = 0
local lastTapTime = 0
...
local function ShowTapFeedback(count)
    task.spawn(function()
        pcall(function()
            ...
            indicator.Size = UDim2.fromOffset(60, 30)
            indicator.Position = UDim2.new(0, MiniButton.AbsolutePosition.X - 5, 0, MiniButton.AbsolutePosition.Y - 35)
            indicator.BackgroundColor3 = Color3.fromRGB(59, 15, 116)
...
local MINI_BUTTON_SIZE = UDim2.fromOffset(50, 50)
...
local function PulseButton(color, scale)
    task.spawn(function()
        pcall(function()
            ...
            local targetSize = UDim2.fromOffset(50 * scale, 50 * scale)
...
UserInputService.InputChanged:Connect(function(input)
    ...
    task.spawn(function()
        task.wait(0.1)
        local totalHeight = 215 + 250 + 20  -- Last section position + height + margin
        MainContentScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, totalHeight)
    end)
...
task.spawn(function()
    while task.wait(0.1) do
        if IsPlaying and CurrentPlayingRecording and ProgressFill then
            local progress = currentPlaybackFrame / #CurrentPlayingRecording
            ProgressFill.Size = UDim2.new(progress, 0, 1, 0)
        elseif IsAutoLoopPlaying and ProgressFill then
            ProgressFill.Size = UDim2.new(0, 0, 1, 0)
        end
    end
end)

-- ========= MINI BUTTON STATUS UPDATE =========
local function UpdateMiniButtonStatus()
    ...
    SafeCall(function()
        ...
        elseif #RecordingOrder > 0 then
            statusText = tostring(#RecordingOrder)
            statusColor = Color3.fromRGB(100, 150, 255)
        end
        
        MiniButton.Text = statusText
        MiniButton.TextColor3 = statusColor
    end)
end
